...
int main(int argc,char** argv){
  ...
  char* send_message=malloc(message_size_B);
  char* recv_message=malloc(message_size_B);

  int dest=(rank+1) % size;
  int source=(rank-1+size)%size; 
  
  fprintf(stderr,"Rank %d started send to %d\n",rank,dest);
  MPI_Sendrecv(send_message,message_size_B,MPI_BYTE,dest,0,recv_message,message_size_B,MPI_BYTE,source,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
  fprintf(stderr,"Rank %d sent message to rank %d\n",rank,dest);

  //fprintf(stderr,"Rank %d started recv from %d\n",rank,source);
  //MPI_Sendrecv(recv_message,message_size_B,MPI_BYTE,source,0,MPI_COMM_WORLD,send_message,message_size_B,MPI_BYTE,rank,0,MPI_COMM_WORLD,MPI_IGNORE_STATUS);
